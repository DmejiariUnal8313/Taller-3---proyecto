...
class InterfazGrafica:
    ...
    def calcular_estado_futuro_BC(self):
        ...
        EstadoFuturo_ABC[:, 2:] = marginalizar_y_normalizar(self.matrices[1], [1, 2], [1, 2])
    
        # Asegúrate de que las dimensiones de EstadoFuturo_ABC[:, 2:] sean (2^2, 2^2)
        if EstadoFuturo_ABC[:, 2:].shape == (2 ** 2, 2 ** 2):
            # Marginalización y normalización sobre el estado futuro BC
            EstadoFuturo_BC[:, 1:] = marginalizar_y_normalizar(EstadoFuturo_ABC[:, 2:], [0], [1, 2])
        else:
